...
最近一次编译/烧录通过：程序 1,146,476 bytes（87% / 1,310,720），全局变量 75,220 bytes（22% / 327,680）；COM3 写入 1,146,864 bytes，Hash verified。
...
录音列表
 ...
 ├─ Ctrl+键 → 给该录音绑定快捷键，并移动到 SD:/IMPORTANT
...
- A线统一为 60 个亮绿竖向小波形柱：录制页直接每帧推进 1 柱；播放页改为按 60 个柱快速抽样，每柱取 3 个 256-sample 小片并保留最大绝对波峰，几乎固定时间铺满整条 A线，而不是跟播放进度慢慢全量扫描。
...
- 新录音编号使用 `nextRecHint` 缓存，并同时避开 `/REC` 与 `/IMPORTANT` 中已有编号，减少录音多时的启动等待。
...
- 列表删除走满进度后先显示 `DEL`，删除后只从内存列表移除该条；不再每次删除后重新扫描 SD，且只有删除了绑定快捷键时才重写 `/IMPORTANT/keys.txt`。
- 快捷键直播放仍保留，适合高频音效/录音触发。
- 重要录音目录：绑定快捷键或按 `Ctrl+Enter` 标记重要时，文件从 `/REC` 移到 `/IMPORTANT`；列表仍统一显示普通+重要录音，重要项在编号后显示 `*`。这样清空普通 `/REC` 时不会影响重要录音。
...
### 录音文件
- 路径：普通录音在 SD 卡 `/REC/REC_XXXX.wav`，重要录音在 `/IMPORTANT/REC_XXXX.wav`，最多 9999 条
- 格式：16kHz、16-bit、单声道 PCM WAV
- 快捷键映射：`/IMPORTANT/keys.txt`，格式为每行 `键,编号`；旧版 `/REC/keys.txt` 会在启动时迁移读取并写入新位置
