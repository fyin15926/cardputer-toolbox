...
最近一次编译/烧录通过：程序 1,143,712 bytes（87% / 1,310,720），全局变量 73,700 bytes（22% / 327,680）；COM3 写入 1,144,096 bytes，Hash verified。
...
录音中
 ...
 ├─ 回车  → 停止并保存 → 录音列表
 ├─ Esc   → 停止并保存 → 息屏
 ├─ 长按 Del 2秒 → 进度条走满后取消并删除本条 → 息屏
 └─ W/S   → 软件增益调节

录音列表
 ├─ ;/.   → 上下选择
 ├─ 回车  → 播放
 ├─ 长按 Del 2秒 → 进度条走满后删除选中录音
...
播放中
 ...
 ├─ +/-   → 音量
 ├─ 空格  → 返回去录音
 └─ 长按 Del 2秒 → 进度条走满后删除当前录音 → 返回列表
```

### UI 简化原则（2026-05-23 更新）
- 已移除右侧三段标签栏，所有界面使用 240px 全宽内容区。
- 开机/空格唤醒不再停留 READY 页，`setup()` 后第一次 `loop()` 会直接进入 `recordingScreen()` 开始录音。
...
- 播放页不再进入前完整扫描 WAV 生成 A线；按下播放后先启动声音，A线随播放缓冲逐步填充，避免 SD 预读造成约 1 秒等待。
- 播放页改为 Canvas 双缓冲推屏，减少 A/B 线和计时器的直接擦屏频闪。
- 播放 A线使用当前播放缓冲增量平均，并做轻微阻尼，避免缩略波形比录制滚动波形显得过高。
...
- 删除操作统一为长按 Del 2 秒：录音中删除本条、播放页删除当前录音、列表页删除选中录音；按住时显示红色进度条和“删除中”，松开即取消。
...
- 统一交互：**录音中空格暂停/继续、回车保存进列表、Esc保存并息屏；列表回车播放选中录音；播放中回车暂停/继续、退格回列表、;/.切换上一条/下一条、Esc息屏**。
