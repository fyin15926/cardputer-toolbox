...
最近一次编译/烧录通过：程序 1,145,088 bytes（87% / 1,310,720），全局变量 73,700 bytes（22% / 327,680）；COM3 写入 1,145,472 bytes，Hash verified。
...
- 播放 A线使用后台预览 + 当前播放缓冲增量平均，并画成竖向小波形柱，既保留整段信息感，又避免缩略波形比录制滚动波形显得过高。
