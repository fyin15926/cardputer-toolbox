...
最近一次编译/烧录通过：程序 1,144,976 bytes（87% / 1,310,720），全局变量 74,724 bytes（22% / 327,680）；COM3 写入 1,145,360 bytes，Hash verified。
...
- 播放 A线只使用后台连续音频块扫描这一套数据源生成，不再被播放缓冲二次改写；录制页和播放页都使用同款亮绿竖向小波形柱，且共用同一套振幅倍率，避免波峰口径不一致。
