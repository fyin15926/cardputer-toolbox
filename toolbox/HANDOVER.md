...
最近一次编译/烧录通过：程序 1,145,220 bytes（87% / 1,310,720），全局变量 73,972 bytes（22% / 327,680）；COM3 写入 1,145,600 bytes，Hash verified。
...
- 列表删除走满进度后先显示 `DEL`，删除后只从内存列表移除该条；不再每次删除后重新扫描 SD，且只有删除了绑定快捷键时才重写 `keys.txt`。
