...
最近一次编译/烧录通过：程序 1,144,972 bytes（87% / 1,310,720），全局变量 74,724 bytes（22% / 327,680）；COM3 写入 1,145,360 bytes，Hash verified。
...
- 播放 A线只使用后台连续音频块扫描这一套数据源生成，不再被播放缓冲二次改写；整条线统一亮绿竖向小波形柱，播放头负责表示进度，避免未播放段和已播放段样式/形态不一致。
