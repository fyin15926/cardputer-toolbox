...
最近一次编译/烧录通过：程序 1,144,956 bytes（87% / 1,310,720），全局变量 73,972 bytes（22% / 327,680）；COM3 写入 1,145,344 bytes，Hash verified。
...
- A线统一为 60 个亮绿竖向小波形柱：录制页直接每帧推进 1 柱，播放页后台按 256 样本块预览并保留每柱最大绝对波峰；录制/播放共用同一套振幅倍率，避免波峰口径不一致。
