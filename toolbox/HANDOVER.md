...
最近一次编译/烧录通过：程序 1,142,736 bytes（87% / 1,310,720），全局变量 73,636 bytes（22% / 327,680）；COM3 写入 1,143,120 bytes，Hash verified。
...
- 播放页 Del 也采用二次确认：第一次显示“再按Del删除”，第二次删除当前 WAV 和对应快捷键绑定，并返回列表。
