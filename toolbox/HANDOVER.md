...
最近一次编译/烧录通过：程序 1,145,580 bytes（87% / 1,310,720），全局变量 73,700 bytes（22% / 327,680）；COM3 写入 1,145,968 bytes，Hash verified。
...
- 播放 A线使用后台多点预览 + 当前播放缓冲增量平均，并画成统一亮绿的竖向小波形柱；播放头负责表示进度，避免未播放段和已播放段样式不一致。
