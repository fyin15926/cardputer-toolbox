...
- 上传加速策略：超过 3MB 的录音在小机器端用 IMA ADPCM 临时压缩后上传，服务器收到后还原成标准 WAV；短录音仍直接上传原始 WAV，避免小文件增加不必要处理。
- 2026-05-25 真机验证：长录音后台显示 `ADPCM` 且速度明显提升，最终可正常转写并发送 flomo；短录音后台显示 `WAV`，不压缩且流程正常。
- 云端后台页面：`http://cardputer.flye.cc/dashboard`。浏览器输入 `UPLOAD_TOKEN` 后可看服务健康、正在上传进度、最近 jobs、设备最近一次上传带来的 Wi-Fi RSSI/IP。
